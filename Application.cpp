...
void Application::processEvent(const SDL_Event &ev)
{
	...
	case SDL_KEYDOWN:
	{
		//Changed keys to make the controlls feel more intuative.
...
void Application::setupScene()
{
	...
	// change the angle of the camera to check if task 2 works
...
void Application::update()
{
	for (auto object : m_objects)
	{
		if (object->m_isDynamic)
		{
			...
			////Pos.z += 1;
			//Matrix3D matric = Matrix3D();
			//matric(3, 0) = Pos.x;
			//matric(3, 1) = Pos.y;
			//matric(3, 2) = Pos.z;

			//object->applyTransformation(matric);
